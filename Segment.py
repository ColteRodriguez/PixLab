...
class PaintApp:
    ...
    def on_backspace(self, event):
        ...
        if self.current != None:
            ...
            if len(self.current.points) >= 2:  # Check if at least 2 dots remain
                prev_dot = self.current.getRecent()  # Get coordinates of second-last dot
                prev_dot_x, prev_dot_y = prev_dot.x, prev_dot.y
                self.delete_item(prev_dot_x+3, prev_dot_y+3, prev_dot_x+3, prev_dot_y+3)  # Delete connecting line
                self.draw_dot(prev_dot_x, prev_dot_y) # add back in the second-to-last dot
            ...
            # elif not self.current.isComplete(): # If the shape is uncompleted after the deletion set to None
            #     self.current = None
            #     self.prev_x = None
            #     self.prev_y = None
    ...
    def on_click(self, event):
        ...
        if self.current == None:
            ...
            if self.current.isComplete():
                ...
                self.draw_dot(x, y)
                self.fill_polygon(self.current)
            else:
                self.draw_dot(x, y)
    ...
    def fill_polygon(self, polygon):
        ...
        draw.polygon(points, fill=(0, 128, 20, 128))  # RGBA color with transparency
